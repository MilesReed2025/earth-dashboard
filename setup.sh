...
# Detect if docker needs sudo (user not in docker group and not root)
docker_sudo() {
  if [[ "$EUID" -eq 0 ]]; then
    echo ""                          # root: no prefix needed
  elif docker info &>/dev/null 2>&1; then
    echo ""                          # user already has access
  elif sudo -n docker info &>/dev/null 2>&1; then
    echo "sudo"                      # sudo works non-interactively
  else
    echo "sudo"                      # ask for password when first used
  fi
}

DOCKER_SUDO="$(docker_sudo)"
# Wrap docker / docker compose to prepend sudo when needed
docker_cmd()  { $DOCKER_SUDO docker "$@"; }
dc_cmd()      { $DOCKER_SUDO $DC "$@"; }

# Check CWD is writable; if not, try sudo tee
write_file() {
  local dest="$1"; shift
  if [[ -w "$(dirname "$dest")" ]]; then
    cat > "$dest"
  else
    warn "$(dirname "$dest") is not writable — using sudo to create $dest"
...
if command -v docker &>/dev/null; then
  ok "Docker $(docker_cmd --version | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)"
  [[ -n "$DOCKER_SUDO" ]] && warn "Docker requires sudo — commands will run with sudo automatically"
else
...
# ── Directory check ──────────────────────────────────────────────────────────
if [[ ! -w "." ]]; then
  warn "Current directory is not writable by you."
  warn "Either cd to a writable location or re-run with: sudo bash setup.sh"
  if ! sudo -n true 2>/dev/null; then
    echo ""
    err "Fix directory permissions before continuing:"
    printf "  ${GRAY}  sudo chown -R \$(whoami) .${RESET}\n"
    echo ""
    exit 1
  fi
fi
...
if [[ -f "earth.yaml" ]]; then
  ...
  if [[ -f "earth.yaml.example" ]]; then
    ...
    sed \
      ...
      earth.yaml.example | write_file earth.yaml
    if [[ -n "$LAT" && -n "$LNG" ]]; then
...
wait $UP_PID && ok "Container started" || { err "docker compose up failed — run: $DOCKER_SUDO $DC logs"; exit 1; }
...
if [[ $ATTEMPTS -ge 30 ]]; then
  ...
  info "Check logs with: ${BOLD}$DOCKER_SUDO $DC logs -f${RESET}"
else
